...
def merge_lcov(lcov_cmd: list[str]) -> None:
    ...
    """
    js_lcov = ROOT_DIR / "coverage" / "js" / "lcov.info"
    python_lcov = ROOT_DIR / "coverage" / "py" / "lcov.info"
    merged_lcov = ROOT_DIR / "coverage" / "lcov.info"

    if not js_lcov.is_file() or not python_lcov.is_file():
        print("Not all lcov files found. Skipping merge.")
        keep_any_lcov(js_lcov, python_lcov)
        return

    # Only merge if Python LCOV is newer
    if python_lcov.stat().st_mtime <= js_lcov.stat().st_mtime:
        print("Python lcov is not newer than JS lcov. Skipping merge.")
        return
