...
const jsLcov = path.resolve(__rootDir, "coverage", "js", "lcov.info");
...
const shouldMerge = () => {
    if (!fs.existsSync(jsLcov) || !fs.existsSync(pythonLcov)) {
        console.log("Not all lcov files found. Skipping merge.");
        return false;
    }

    const jsTime = fs.statSync(jsLcov).mtime;
    const pythonTime = fs.statSync(pythonLcov).mtime;

    if (jsTime <= pythonTime) {
        console.log("JS lcov is not newer than Python lcov. Skipping merge.");
        return false;
    }
    return true;
};
